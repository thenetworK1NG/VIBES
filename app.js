...
loadFirebase(() => {
  ...
  if (menuEditProfile) {
    menuEditProfile.onclick = () => {
      ...
      db.ref('users/' + currentUser).once('value', snap => {
        ...
        renderEditAvatarOptions(user && user.avatar ? user.avatar : AVATARS[0]);
    ...
    db.ref('users').once('value', snapshot => {
      ...
      Object.keys(users).forEach(user => {
        if (user !== currentUser && user.toLowerCase().includes(search)) {
          ...
          db.ref('messages/' + chatId).once('value', msgSnap => {
            let unreadCount = 0;
            msgSnap.forEach(msg => {
              const val = msg.val();
              if (val.receiver === currentUser && !val.read) unreadCount++;
            });
            if (unreadCount > 0) {
              const bubble = document.createElement('span');
              bubble.className = 'user-unread-bubble';
  ...
  // Helper: Render avatar options for edit modal
  ...
  saveProfileBtn.onclick = function() {
    ...
    if (newUsername !== oldUsername) {
        ...
        if (snap.exists()) {
          ...
          db.ref('users/' + oldUsername).once('value', userSnap => {
            ...
            db.ref('messages').once('value', msgSnap => {
              ...
              profileModal.style.display = 'none';
              // Update UI in-place
              updateProfileUI(newUsername, newAvatar);
            });
          });
        }
      });
      ...
      db.ref('messages').once('value', msgSnap => {
        ...
        profileModal.style.display = 'none';
        // Update UI in-place
        updateProfileUI(oldUsername, newAvatar);
      });
    }
  };

  // Helper to update nav avatar and user list after profile change
  function updateProfileUI(username, avatar) {
    // Update nav avatar
    navProfileAvatar.src = avatar;
    // Update user list
    loadUserList(username, navSearch.value.trim().toLowerCase());
    // If chat with self is open, update avatar there too
    if (document.getElementById('chat-with')) {
      const chatWith = document.getElementById('chat-with');
      if (chatWith.innerHTML.includes(username)) {
        chatWith.innerHTML = `<img src="${avatar}" class="avatar"> Chat with ${username}`;
      }
    }
  }
